...
export class UserService {
  ...
  async findAllDeanGeneralAdministration(): Promise<User[]> {
    const users = this.userRepository
      .createQueryBuilder('user')
      .select(['user.user_name', 'user.user_role', 'user.user_id'])
      .leftJoinAndSelect('user.works', 'work')
      .leftJoinAndSelect('work.options', 'option')
      .where(
        'user.user_role = :user_role OR user.user_role = :user_role2',
        {
          user_role: 'หัวหน้างานฝ่ายบริหารทั่วไป',
          user_role2: 'พนักงานฝ่ายบริหารทั่วไป',
        },
      )
      .getMany();
    return users;
  ...
  async findAllDeanAcademic(): Promise<User[]> {
    const users = this.userRepository
      .createQueryBuilder('user')
      .select(['user.user_name', 'user.user_role', 'user.user_id'])
      .leftJoinAndSelect('user.works', 'work')
      ...
      .where(
        'user.user_role = :user_role OR user.user_role = :user_role2',
        {
          user_role: 'หัวหน้างานฝ่ายวิชาการ',
          user_role2: 'พนักงานฝ่ายวิชาการ',
        },
      )
      .getMany();
    return users;
  ...
  async findAllDeanPlan(): Promise<User[]> {
    const users = this.userRepository
      .createQueryBuilder('user')
      .select(['user.user_name', 'user.user_role', 'user.user_id'])
      .leftJoinAndSelect('user.works', 'work')
      ...
      .where(
        'user.user_role = :user_role OR user.user_role = :user_role2',
        {
          user_role: 'หัวหน้างานฝ่ายแผนงาน',
          user_role2: 'พนักงานฝ่ายแผนงาน',
        },
      )
      .getMany();
    return users;
  ...
  async findAllDeanDevelopDigitalSkills(): Promise<User[]> {
    const users = this.userRepository
      .createQueryBuilder('user')
      .select(['user.user_name', 'user.user_role', 'user.user_id'])
      .leftJoinAndSelect('user.works', 'work')
      ...
      .where(
        'user.user_role = :user_role OR user.user_role = :user_role2',
        {
          user_role: 'หัวหน้างานฝ่ายพัฒนาทักษะดิจิทัล',
          user_role2: 'พนักงานฝ่ายพัฒนาทักษะดิจิทัล',
        },
      )
      .getMany();
    return users;
